...
-- Create skills table first (since it was missing)
...
SET description = 
  CASE 
    WHEN company = 'Accenture Private Limited' THEN
    ...
    WHEN company = 'Jaldi Technologies Pvt Ltd' THEN
    ...
    ELSE description
  END
WHERE company IN ('Accenture Private Limited', 'Jaldi Technologies Pvt Ltd', 'Flameingoes');
...
SET 
  title = 
    CASE 
      WHEN id = (SELECT id FROM projects LIMIT 1) THEN 'BASF Monterey Carveout'
      ELSE 'Smart Street Lights System'
    END,
  description = 
    CASE 
      WHEN id = (SELECT id FROM projects LIMIT 1) THEN
